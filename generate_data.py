...
STATUSES = ['Completed', 'Active', 'Delayed', 'Over Budget']
...
for i in range(NUM_PROJECTS):
    ...
    cost_overrun_factor = np.random.uniform(0.75, 1.25)
    ...
    delay_days = int(np.random.uniform(-45, 45))
    actual_end = planned_end + timedelta(days=delay_days)

    # Change orders
    change_order_count = random.randint(0, 8)
    change_order_cost = round(budgeted_cost * random.uniform(0, 0.18) 
...
# Export
df.to_csv(r'C:\Users\prest\Desktop\Business & Data Analysis\Portfolio Projects\Oil & Gas\Oil and Gas Capital Project Portfolio\oil_and_gas_project_data.csv', index=False)
